...
def main():
    ...
    count_parser.add_argument(
        "--output", "-o", help="Path where the word count results will be saved"
    )
    count_parser.add_argument(
        "--postgres", "-p", action="store_true", help="Save results to PostgreSQL"
    )
    count_parser.add_argument(
        "--update", "-u", action="store_true", help="Update existing word counts in PostgreSQL"
    )
    count_parser.add_argument(
        "--driver-memory", default="4g", help="Driver memory allocation (default: 4g)"
    )
    count_parser.add_argument(
        "--executor-memory", default="4g", help="Executor memory allocation (default: 4g)"
    )
    count_parser.add_argument(
        "--max-result-size", default="2g", help="Maximum result size (default: 2g)"
    )
    count_parser.add_argument(
        "--batch-size",
        type=int,
    ...
    web_parser.add_argument(
        "--debug", action="store_true", help="Run the web application in debug mode"
    )
    ...
    if (
        args.command == "count" or args.command is None
    ):  # Default to count for backward compatibility
        # Validate input path
